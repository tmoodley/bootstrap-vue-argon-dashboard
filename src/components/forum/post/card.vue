    ...
    <div class="card gedf-card">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="ml-2">
              <div class="h5 m-0">@{{post.organizations.name}}</div>
      ...
      <div class="card-body">
        ...
        <div v-if="post.tags != null">
          <span v-for="tag in post.tags.split(',')" class="badge badge-primary" :key="tag.id">{{tag}}</span>
        </div>
      </div>
      <div class="card-footer">
        ...
        <b-list-group v-show="showComments">
          <b-list-group-item active class="flex-column align-items-start" v-for="comment in post.comments" :key="comment.id">
            <b-row>
              <b-col cols="1">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      computedUrl() {
        return "/uploads/" + JSON.parse(localStorage.getItem('userId')) + "/" + this.partnerStore.displayPicture;
      },
      computedUrlOrg() {
        return "/uploads/" + JSON.parse(localStorage.getItem('organizationId')) + "/" + this.companystore.logo;
      },
      isUserLiked() {
        var isLiked = false;
        var self = this;
